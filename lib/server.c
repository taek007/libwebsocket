	...
	case LWSCM_SERVER_LISTENER:
		...
		do {
			...
			accept_fd  = accept(pollfd->fd, (struct sockaddr *)&cli_addr,
					    &clilen);
			//todo
			lwsl_notice("函数:%s 服务器接收到来自客户端的socket fd: %d\n",__func__,accept_fd);
			...
			lwsl_notice("accepted new conn  port %u on fd=%d\n",
...
lws_interpret_incoming_packet(struct lws *wsi, unsigned char **buf, size_t len)
{
	...
	printf("lws_interpret_incoming_packet: received %d byte packet\n",  (int)len);
#if 0
	...
	while (len) {
		printf("the lws_interpret_incoming_packet len is %d",len);
		/*
		...
		if (wsi->u.ws.rx_draining_ext) {
			printf("wsi->u.ws.rx_draining_ext\n");
			m = lws_rx_sm(wsi, 0);
			if (m < 0)
				return -1;
			continue;
		...
		/* process the byte */
		printf("lws_interpret_incoming_packet call lws_rx_sm\n");
		m = lws_rx_sm(wsi, *(*buf)++);
		if (m < 0)
			return -1;
		len--;
	...
	lwsl_notice("lws_interpret_incoming_packet exit with %d unused\n", (int)len);
